...
| `on_reset_command` | `""` | Команда оболонки (shell), що виконується при скиданні лімітів (наприклад, запуск скрипта) |
| `on_threshold_command` | `""` | Команда оболонки (shell), що виконується при досягненні порогу використання |

> [!TIP]
> Команди виконуються у робочій директорії програми. Ви можете розмістити власні скрипти (наприклад, [`claude_reset_hook.ps1`](claude_reset_hook.ps1)) та файл [`.env`](.env.example) безпосередньо поруч із портативним EXE-файлом.
